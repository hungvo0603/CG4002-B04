...
bool handshake()
{ // wait for ACK from laptop to establish 3 way handshake
...
char packetACK()
{ // Waiting for ACK/NAK after sending packet
  if (Serial.available())
  {
    char response = Serial.read();
...
void addSeqIDToPacket(byte pkt[], char SeqID)
{                // not used
...
typedef union
{ // define a type with the following attributes
...
void loop()
{
  ...
  if ((millis() - serialPreviousTime) > kSerialInterval)
  {
    ...
    if (counter < 40)
    { // 2 seconds recording time = 40 * kSerialInterval
